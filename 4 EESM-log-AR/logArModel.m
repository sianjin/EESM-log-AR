...
% effSnrVecLog_e = effSnrVecLog(1:end); % Estimation data
% effSnrVecLog_v = effSnrVecLog(end/2+1:end); % Validation data
%% Model order selection
% % Package estimation data into Iddata format
% effSnrVecLogId_e = iddata(effSnrVecLog_e,[],254/10^6); % Sample time = 254us
% effSnrVecLogId_v = iddata(effSnrVecLog_v,[],254/10^6); % Sample time = 254us
% V = arxstruc(effSnrVecLogId_e,effSnrVecLogId_v,(1:100)'); % Checking all order up to 40
% order = selstruc(V,0); % select the structure with the smallest loss function
% % order = selstruc(V,'AIC'); % find the structure that minimizes Akaike's Information Criterion
% % order = selstruc(V,'MDL');% find the structure that minimizes the Rissanen's Minimum Description Length
%% Plot ACF and PACF of effective SNR (log scale)
figure
subplot(2,1,1)
autocorr(effSnrVecLog,'NumLags',40)
subplot(2,1,2)
parcorr(effSnrVecLog,'NumLags',40) 
